...
# run app
java -jar target/super-shah-java-1.0-SNAPSHOT.jar
...
docker buildx build   --platform linux/amd64   -t parsontodd/super-shah-java:latest   --push .
```
...
# watch rollout
kubectl rollout status deploy/super-shah-java -n care

# get external endpoint
kubectl get svc super-shah-java-svc -n care
```
...
```bash
kubectl port-forward svc/super-shah-java-svc 8080:80 -n care
...
kubectl rollout restart deploy/super-shah-java -n care
kubectl rollout status deploy/super-shah-java -n care
```
...
├─ src/
│  ├─ main/
│  │  ├─ java/com/harness/supershahjava/
│  │  │  ├─ App.java
...
│  │        └─ index.html
│  └─ test/
│     └─ java/com/harness/supershahjava/AppTest.java
├─ kubernetes/
│  ├─ deployment.yml
│  └─ service.yml
├─ Dockerfile
...
© chintan.shah@carelumen.com. For demo/PoV purposes.
